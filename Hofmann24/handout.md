...
* Humans generalize over word types, while GPT-J and GPT-4 rely on word tokens. This difference impacts how morphological complexity (e.g., recognizable prefixes and suffixes) contributes to perceived *familiarity*.
* Contrast results on morphologically parsable words (identifiable affixes and stems) vs. simplex words (cannot be decomposed into smaller morphological components).
* Humans: Morphologically parsable words are rated as more familiar than simplex words, even if rare.
* GPT-J: Parsable words are assigned lower familiarity than simplex words, contradicting human behavior. The model's familiarity ratings are strongly tied to word frequency.
* Key Finding: The lack of dissociation between frequency and familiarity in GPT-J underscores its inability to mimic human type-level processing. Morphological complexity plays a key role in human perception of word familiarity but is not utilized by GPT-J in the same way.

### Conclusions
* LLMs do analogical rather than rule-based generalization.
* Key differences in human vs. LLM lexicons.
* Limitations in human-like skills by LLMs.
